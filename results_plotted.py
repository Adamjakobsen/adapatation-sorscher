...
def plot_training_metric_std_err(key_list, metric_name, metric_name_pretty):
    ...
    for key in key_list:
        ...
        data_dict[key] = [mean - std_error, mean, mean + std_error]

    plt.figure()
    ax = plt.gca()
    for key in key_list:
        ax.fill_between(
            np.arange(len(data_dict[key][0])), data_dict[key][0], data_dict[key][2], color=colors[key] + "55"
        )
...
def get_metrics(key_list: list, N_silenced: int = 1, bin_num: int = 30) -> dict:
    ...
    metric_data = {}
    for key in key_list:
        metric_data[key] = {"difference": [], "mean": []}
