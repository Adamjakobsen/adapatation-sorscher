...
def test_robustness():
    ...
    N_silenced = 1
    difference_vanilla, mean_vanilla, mean_rnn_vanilla = get_robustness(
        model_vanilla, (velocities, init_pc_positions, positions), dataset, N_silenced
    )
    difference_weight, mean_weight, mean_rnn_weight = get_robustness(
        model_weight, (velocities, init_pc_positions, positions), dataset, N_silenced
    )
    difference_adapt, mean_adapt, mean_rnn_adapt = get_robustness(
        model_adapt, (velocities, init_pc_positions, positions), dataset, N_silenced
    )
